...
I used `ggplot2` (and some other packages) to create the visualisation. My idea was to show the variation across provinces using a choropleth map: as Ecuador has three distinct geographical regions, I thought there might emerge a clear visual pattern along regional lines if there exists one. Besides the map, I decided to add all data points (with the names of provinces as labels) to show the variation better and to make comparing individual provinces easier.
...
There seems to be some variation (range: 0.9 - 2) across provinces, with most of them having between 1 and 1.5 hospital beds per 1000 people. Is this variation large relative to other countries? It's difficult to know for sure, but the variation doesn't seem particularly large. For example, the same statistic ranges from 1.7 to 4.8 in [US states](https://www.kff.org/other/state-indicator/total-hospital-beds/).

Most important, however, than a mere statistic is what this variability reflects about access to health care services in different parts of Ecuador. Unfortunately, this data doesn't speak to this issue---maybe that's something to look into in a future post.
